...
-- Function to calculate end_date for events
CREATE OR REPLACE FUNCTION calculate_event_end_date()
RETURNS TRIGGER AS $$
BEGIN
    NEW.end_date = NEW.start_date + NEW.duration;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to automatically calculate end_date
CREATE TRIGGER calculate_event_end_date_trigger
    BEFORE INSERT OR UPDATE OF start_date, duration ON events
    FOR EACH ROW EXECUTE FUNCTION calculate_event_end_date();
